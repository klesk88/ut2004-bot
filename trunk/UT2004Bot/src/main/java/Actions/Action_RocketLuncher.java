 ...
 * @author klesk
 */
public class Action_RocketLuncher implements Action
{
     private float confidence = 0.8f; 
    
  public Action_RocketLuncher() {
    ...
    public WorldState.TruthStates[] getPreConditionArray() {
        ...
        preConditionArray[WorldState.Symbols.HasRocketAmmunition.ordinal()] = WorldState.TruthStates.True;
    ...
    public Action.ActionResult executeAction() {
        ...
        if (BotLogic.getInstance().getWeaponry().getCurrentWeapon().getType() == ItemType.ROCKET_LAUNCHER) {
           
        } else {
          
            BotLogic.getInstance().writeToLog_HackCosIMNoob("changing to rocket luncher");
            BotLogic.getInstance().getShoot().changeWeapon(ItemType.ROCKET_LAUNCHER);
        }
