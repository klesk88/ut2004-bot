...
public class Action_FindLightiningGunAmmo implements Action{
    ...
    public Action.ActionResult executeAction() {
        ...
        if(! BotLogic.getInstance().getWeaponry().hasWeapon(ItemType.LIGHTNING_GUN))
        {
            ...
            Set<UnrealId> itemSet = BotLogic.getInstance().getItems().getAllItems(ItemType.LIGHTNING_GUN).keySet();

            Iterator it = itemSet.iterator();

            while (it.hasNext()) {
                UnrealId value = (UnrealId) it.next();
                bb.targetPos = BotLogic.getInstance().getItems().getAllItems(ItemType.LIGHTNING_GUN).get(value).getLocation();
        ...
        if (!currentlySearchingAmmo) {
            ...
            Set<UnrealId> itemSet = BotLogic.getInstance().getItems().getAllItems(ItemType.LIGHTNING_GUN_AMMO).keySet();
            ...
            while (it.hasNext()) {
               // BotLogic.getInstance().writeToLog_HackCosIMNoob("found shock ammunition");
                UnrealId value = (UnrealId) it.next();
                bb.targetPos = BotLogic.getInstance().getItems().getAllItems(ItemType.LIGHTNING_GUN_AMMO).get(value).getLocation();
