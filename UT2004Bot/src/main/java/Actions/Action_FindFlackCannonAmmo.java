 ...
 * @author klesk
 */
public class Action_FindFlackCannonAmmo implements Action{
    
     private float confidence = 1f;       
           boolean currentlySearchingAmmo = false;
     
    public Action_FindFlackCannonAmmo() {
    ...
    public Action.ActionResult executeAction() {
        ...
        if(! BotLogic.getInstance().getWeaponry().hasWeapon(ItemType.FLAK_CANNON))
        {
            BlackBoard bb = BlackBoard.getInstance();

            Set<UnrealId> itemSet = BotLogic.getInstance().getItems().getAllItems(ItemType.FLAK_CANNON).keySet();

            Iterator it = itemSet.iterator();

            while (it.hasNext()) {
                UnrealId value = (UnrealId) it.next();
                bb.targetPos = BotLogic.getInstance().getItems().getAllItems(ItemType.FLAK_CANNON).get(value).getLocation();
        ...
        if (!currentlySearchingAmmo) {
            ...
            Set<UnrealId> itemSet = BotLogic.getInstance().getItems().getAllItems(ItemType.FLAK_CANNON_AMMO).keySet();
            ...
            while (it.hasNext()) {
               // BotLogic.getInstance().writeToLog_HackCosIMNoob("found shock ammunition");
                UnrealId value = (UnrealId) it.next();
                bb.targetPos = BotLogic.getInstance().getItems().getAllItems(ItemType.FLAK_CANNON_AMMO).get(value).getLocation();
        ...
        } else if (BotLogic.getInstance().getWeaponry().getAmmo(ItemType.FLAK_CANNON_AMMO) > 5) {
